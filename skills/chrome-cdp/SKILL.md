...
- If nothing responds, it launches a dedicated Chrome instance (`--remote-debugging-port=9222 --user-data-dir=~/.cache/chrome-cdp-profile`) and waits up to 15s for the port; site logins persist in that profile across restarts

### Auto-launch options

`CDP_AUTOLAUNCH_PORT` (default `9222`), `CDP_PROFILE_DIR` (default `~/.cache/chrome-cdp-profile`), `CDP_NO_AUTOLAUNCH=1` to disable.
...
- A background daemon keeps the CDP session alive and auto-exits after 20 minutes of inactivity; re-attach is automatic.
